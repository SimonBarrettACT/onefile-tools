...
class Provider extends REST_Controller {
	...
	public function index_post() {
        ...
        $json = $this->provider->createProvider($parameters);
        ...
        if (is_integer($return))
        {
            $this->set_response([
                'status' => TRUE,
                'id' => $return,
                'message' => 'A new provider has been created'
        ...
        else
        {
            $this->set_response([
                'status' => FALSE,
                'error' => 'A provider could not be created'
    ...
    public function id_get($id)
    {
        ...
        // Find and return a single record for a particular provider.
        ...
        $json = $this->provider->getProvider($id);
        $return = json_decode($json, true);
        ...
        else
        {
            $this->set_response([
                'status' => FALSE,
                'error' => 'Provider could not be found'
            ], REST_Controller::HTTP_NOT_FOUND); // NOT_FOUND (404) being the HTTP response code
        }
    ...
    public function id_post($id)
    {
        ...
        $return = $this->provider->updateProvider($id, $parameters );
    ...
    {
        $parameters = $this->post();
        $json = $this->provider->getProviders($parameters, $pageNumber, $pageSize);
